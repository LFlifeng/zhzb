...
<head>
	...
	<link rel="stylesheet" type="text/css" href="${skin}front/css/registe.css">
...
<body>
			...
			<div class="registe_center r">
				<div class="registe_registe">
					...
					<form action="#" method="post">
						<div class="contacts">
							<span>姓名</span> <input id='contacts' type="text" placeholder="请输入姓名">
						</div>
						<div class="pwd">
							<span>密码</span> <input type="password" name="" id="pwd" placeholder="请输入密码">
						</div>
						<div class="phone">
							<span>手机号</span> <input type="text" name="" id="phone" placeholder="请输入手机号"> <span id='getCode'
						...
						<div class="clause">
							<input type="checkbox" name="" id="clause">我已阅读并接受 <a href="#" onclick="agreement();">《中汇招标网用户服务条款》</a>
							<span id='notice_id' style="color: red; width: 100%; text-align: center;"></span>
						</div>
	...
	<script>
		...
		function registe() {
			...
			var emptysize = 0;
			//遍历选中的checkbox给角色赋值
			$.each($('.who input:checkbox:checked'), function () {
				//当选中一个checkbox时
				role = $(this).val() + role;
			});
			switch (role) {
			...
			if (emptysize == 0) {
				$.ajax({
					...
					success: function (res) {
						if (res == "succ") {
							window.location.href = "userQd.login.do"
